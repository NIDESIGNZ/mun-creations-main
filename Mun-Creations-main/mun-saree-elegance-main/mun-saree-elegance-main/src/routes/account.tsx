    ...
    <div className="container-boutique space-y-8">
      ...
      <div className="bg-white p-6 md:p-8 rounded-sm border border-border shadow-md flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-serif text-2xl font-bold text-[var(--wine-deep)]">{customerProfile.name}</h1>
            <div className="text-xs text-muted-foreground">{customerProfile.email} · Member since {customerProfile.memberSince}</div>
          </div>
        </div>

        <button className="text-xs font-bold text-red-600 hover:underline flex items-center gap-1.5 border border-red-200 px-4 py-2 rounded-sm bg-red-50">
          ...
          <div className="bg-white p-2 rounded-sm border border-border shadow-sm space-y-1 text-xs font-bold uppercase tracking-wider">
            <button
              ...
              className={`w-full text-left p-3 rounded-xs flex items-center gap-2.5 transition-colors ${
                activeTab === "orders" ? "bg-[var(--wine)] text-white" : "hover:bg-secondary text-foreground"
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left p-3 rounded-xs flex items-center gap-2.5 transition-colors ${
                activeTab === "profile" ? "bg-[var(--wine)] text-white" : "hover:bg-secondary text-foreground"
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left p-3 rounded-xs flex items-center gap-2.5 transition-colors ${
                activeTab === "addresses" ? "bg-[var(--wine)] text-white" : "hover:bg-secondary text-foreground"
              }`}
            >
            ...
            <button
              ...
              className={`w-full text-left p-3 rounded-xs flex items-center gap-2.5 transition-colors ${
                activeTab === "wishlist" ? "bg-[var(--wine)] text-white" : "hover:bg-secondary text-foreground"
              }`}
            >
          ...
          {activeTab === "orders" && (
            <div className="bg-white p-6 md:p-8 rounded-sm border border-border shadow-md space-y-6 animate-in fade-in">
              ...
              <div className="space-y-6">
                {orders.map((ord) => (
                  <div key={ord.id} className="p-5 border border-border rounded-sm space-y-4 bg-secondary/10">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-border text-xs gap-2">
                      <div>
                        <div className="font-mono font-bold text-sm text-[var(--wine-deep)]">{ord.id}</div>
                        <div className="text-muted-foreground text-[10px]">Placed on {new Date(ord.createdAt).toLocaleDateString()}</div>
                      </div>
                    ...
                    <div className="space-y-2 text-xs">
                      {ord.items.map((it, idx) => (
                        <div key={idx} className="flex justify-between items-center">
                          <span className="font-serif font-bold">{it.productName} (Qty: {it.quantity})</span>
                          <span className="font-semibold">{formatPrice(it.priceUsd * it.quantity)}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="pt-3 border-t border-border flex flex-col sm:flex-row sm:items-center justify-between text-xs gap-2 text-muted-foreground">
                      <div>
                        Shipping Address: <strong className="text-foreground">{ord.shippingAddress}</strong>
                      </div>
                      {ord.awbNumber && (
                        <a
          ...
          {activeTab === "addresses" && (
            <div className="bg-white p-6 md:p-8 rounded-sm border border-border shadow-md space-y-6 animate-in fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="p-4 border-2 border-[var(--wine)] rounded-sm bg-[var(--wine)]/5 space-y-2 relative">
                  <span className="bg-[var(--wine)] text-white text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">
                    Default Address
                  </span>
                  <div className="font-bold text-foreground">Home Address</div>
                  <div className="text-muted-foreground leading-relaxed">
                    450 Lexington Ave, Suite 1200<br />
          ...
          {activeTab === "wishlist" && (
            <div className="bg-white p-6 md:p-8 rounded-sm border border-border shadow-md space-y-6 animate-in fade-in">
              ...
              <div className="text-xs text-muted-foreground p-6 text-center border border-border rounded bg-secondary/20">
                Your saved wishlist items will appear here. Click the heart icon on any saree to save it for later.
              </div>
            </div>
          )}
        </div>
