...
export const NEW_ARRIVALS = PRODUCTS.filter((p) => p.badge === "new").concat(PRODUCTS).slice(0, 6);
export const BESTSELLERS = PRODUCTS.filter((p) => p.badge === "bestseller").concat(PRODUCTS).slice(0, 6);
export const BRIDAL = PRODUCTS.filter((p) => p.occasion?.includes("Wedding") || p.subcategory?.includes("Bridal"));
